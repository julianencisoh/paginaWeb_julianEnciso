import { initializeApp } from "https://www.gstatic.com/firebasejs/9.3.0/firebase-app.js";
import { getFirestore, doc, setDoc} from "https://www.gstatic.com/firebasejs/9.3.0/firebase-firestore.js";

const app = initializeApp(firebaseConfig);
const db = getFirestore(app);

/*
const productos = [
    {
        id: 1,
        name: "Jarron Oro Rosa",
        price: 12000,
        size: "Mediano",
        image: "./recursos/JarronOroRosa.png",
        ref: "0301",
        description: "Nuestro Jarrón oro rosa es el complemento ideal para tu decoración en tonos tierras, nudes, rosas, o como quieras combinarlo. Puedes llevarlo con las flores artificiales color rosa o decorarlo con flores naturales a tu elección. Te recomendamos el cuidado de este producto y transportarlo en su empaque.",
        type: "Jarrones"
    },
    {
        id: 2,
        name: "Candelabro 3 Niveles",
        price: 10000,
        size: "Mediano",
        image: "./recursos/CandelabroTresNiveles.png",
        ref: "0305",
        description: "Nuestro Jarrón oro rosa es el complemento ideal para tu decoración en tonos tierras, nudes, rosas, o como quieras combinarlo. Puedes llevarlo con las flores artificiales color rosa o decorarlo con flores naturales a tu elección. Te recomendamos el cuidado de este producto y transportarlo en su empaque.",
        type: "Candelabros"
    },
    {
        id: 3,
        name: "Jarron Espejos",
        price: 15000,
        size: "Mediano",
        image: "./recursos/JarronEspejos.png",
        ref: "0302",
        description: "Nuestro Jarrón oro rosa es el complemento ideal para tu decoración en tonos tierras, nudes, rosas, o como quieras combinarlo. Puedes llevarlo con las flores artificiales color rosa o decorarlo con flores naturales a tu elección. Te recomendamos el cuidado de este producto y transportarlo en su empaque.",
        type: "Jarrones"
    },
    {
        id: 4,
        name: "Mesa Vintage",
        price: 30000,
        size: "Grande",
        image: "./recursos/Vintage.png",
        ref: "0309",
        description: "Nuestro Jarrón oro rosa es el complemento ideal para tu decoración en tonos tierras, nudes, rosas, o como quieras combinarlo. Puedes llevarlo con las flores artificiales color rosa o decorarlo con flores naturales a tu elección. Te recomendamos el cuidado de este producto y transportarlo en su empaque.",
        type: "Mesas"
    },
    {
        id: 5,
        name: "Contenedor Cristal",
        price: 11000,
        size: "Pequeño",
        image: "./recursos/ContenedorCristal.png",
        ref: "0303",
        description: "Nuestro Jarrón oro rosa es el complemento ideal para tu decoración en tonos tierras, nudes, rosas, o como quieras combinarlo. Puedes llevarlo con las flores artificiales color rosa o decorarlo con flores naturales a tu elección. Te recomendamos el cuidado de este producto y transportarlo en su empaque.",
        type: "Jarrones"
    },
    {
        id: 6,
        name: "Candelabro París",
        price: 12000,
        size: "Pequeño",
        image: "./recursos/CandelabroParis.png",
        ref: "0304",
        description: "Nuestro Jarrón oro rosa es el complemento ideal para tu decoración en tonos tierras, nudes, rosas, o como quieras combinarlo. Puedes llevarlo con las flores artificiales color rosa o decorarlo con flores naturales a tu elección. Te recomendamos el cuidado de este producto y transportarlo en su empaque.",
        type: "Candelabros"
    },
    {
        id: 7,
        name: "Candelabro Farol",
        price: 10000,
        size: "Pequeño",
        image: "./recursos/CandelabroFarol.png",
        ref: "0306",
        description: "Nuestro Jarrón oro rosa es el complemento ideal para tu decoración en tonos tierras, nudes, rosas, o como quieras combinarlo. Puedes llevarlo con las flores artificiales color rosa o decorarlo con flores naturales a tu elección. Te recomendamos el cuidado de este producto y transportarlo en su empaque.",
        type: "Candelabros"
    },
    {
        id: 8,
        name: "Mesa Diamante",
        price: 25000,
        size: "Grande",
        image: "./recursos/MesaDiamante.png",
        ref: "0307",
        description: "Nuestro Jarrón oro rosa es el complemento ideal para tu decoración en tonos tierras, nudes, rosas, o como quieras combinarlo. Puedes llevarlo con las flores artificiales color rosa o decorarlo con flores naturales a tu elección. Te recomendamos el cuidado de este producto y transportarlo en su empaque.",
        type: "Mesas"
    },
    {
        id: 9,
        name: "Mesa Espiral",
        price: 25000,
        size: "Mediano",
        image: "./recursos/MesaEspiral.png",
        ref: "0308",
        description: "Nuestro Jarrón oro rosa es el complemento ideal para tu decoración en tonos tierras, nudes, rosas, o como quieras combinarlo. Puedes llevarlo con las flores artificiales color rosa o decorarlo con flores naturales a tu elección. Te recomendamos el cuidado de este producto y transportarlo en su empaque.",
        type: "Mesas"
    },
    {
        id: 10,
        name: "Mesa Cúbica",
        price: 25000,
        size: "Mediano",
        image: "./recursos/MesaCubica.png",
        ref: "0310",
        description: "Nuestro Jarrón oro rosa es el complemento ideal para tu decoración en tonos tierras, nudes, rosas, o como quieras combinarlo. Puedes llevarlo con las flores artificiales color rosa o decorarlo con flores naturales a tu elección. Te recomendamos el cuidado de este producto y transportarlo en su empaque.",
        type: "Mesas"
    },

    
];

*/

productos.forEach(async (producto) => {
    await setDoc(doc(db, "productos", `CVS8SDV998VZFESE${producto.id}`), producto);
});